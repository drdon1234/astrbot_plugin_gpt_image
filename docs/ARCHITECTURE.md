...
本项目是一个独立 AstrBot 插件。插件监听所有群聊/私聊消息，由内部解析器识别配置里的生图触发词，解析用户输入中的提示词和末尾分辨率；如果当前消息回复/引用了含图片消息，则提取引用消息里的全部图片作为参考图；否则执行纯文本生图。
...
└── core/
    ...
    ├── constants.py        # 插件名、固定提示文案和图片 MIME 常量
...
默认尺寸由 `defaults.size_preset` 下拉控制，可选 `自动`、像素 + 比例 + 几K 的常用尺寸和 `自定义`。选择 `自定义` 时读取 `defaults.custom_size`。聊天输入只接受两类末尾分辨率：具体像素尺寸，或比例/方向 + 几K 的简写；`自动` 只属于配置默认值，不作为聊天参数解析。
...
聊天中不解析 `自动` / `auto`，也不解析 `1280x720 16:9 1K` 这类配置下拉标签。没有被识别为合法末尾分辨率的文本会完整保留为提示词。
...
* 监听群聊和私聊消息，识别配置里的生图触发词和 `生图额度` 额度查询触发词。
* 初始化插件数据目录。
...
* `string_list()`、`configured_string_list()`、`bool_value()`、`int_value()`：配置类型归一化。
...
生图 一个银白色机械鸟停在玻璃树枝上 1536×1024
生图 一个银白色机械鸟停在玻璃树枝上 1536 x 1024
生图 一个银白色机械鸟停在玻璃树枝上 16:9 2K
生图 一个银白色机械鸟停在玻璃树枝上 横屏 2K
```

解析规则：

* 触发词必须在消息正文最前。
* 如果平台因引用其他用户消息在触发词前自动添加 @，入口层会先忽略这些前置 @。
* 不解析带斜杠的命令写法。
* 触发词后、末尾分辨率前的全部内容都会作为提示词。
* 只有末尾片段能被识别为分辨率。
* 分辨率可以是数字尺寸，或 `16:9 2K` / `横屏 2K` 这类比例/方向简写；数字尺寸分隔符统一归一为 `x`。
* `1280x720 16:9 1K` 这类像素 + 比例 + 几K 组合只用于配置下拉，不作为聊天参数解析。
...
配置只控制默认尺寸选项、自定义分辨率、质量、生图/改图张数、生图触发词和总并发请求数；模型、背景、输出格式和参考图保真度行为是内部固定值。
